...
AUTH_HEADERS = {"Authorization": "Bearer secret", "Idempotency-Key": "test-idempotency-key"}
...
def test_execute_requires_authentication(monkeypatch):
    ...
    response = client.post("/v1/cybersecurity/execute", headers={"Idempotency-Key": "test-idempotency-key"}, json={"tenant_id": "org123", "payload": BASE_PAYLOAD})
    assert response.status_code == 401


def test_execute_requires_idempotency_key(monkeypatch):
    configure_static_test_auth(monkeypatch)
    response = client.post("/v1/cybersecurity/execute", headers={"Authorization": "Bearer secret"}, json={"tenant_id": "org123", "payload": BASE_PAYLOAD})
...
def test_execute_requires_upstream(monkeypatch):
    ...
    monkeypatch.setenv("FDE_MASTER_UPSTREAM_TOKEN", "static-token")
    response = client.post("/v1/cybersecurity/execute", headers=AUTH_HEADERS, json={"tenant_id": "org123", "payload": BASE_PAYLOAD})
...
def test_execute_rejects_unknown_domain(monkeypatch):
    configure_static_test_auth(monkeypatch)
    response = client.post("/v1/not-a-real-domain/execute", headers=AUTH_HEADERS, json={"tenant_id": "org123", "payload": BASE_PAYLOAD})
...
def test_execute_calls_fde_mastery_canonical_route_and_envelope(monkeypatch):
    configure_static_test_auth(monkeypatch)
    route = respx.post("https://fde-mastery.internal/v1/cybersecurity/execute").mock(return_value=Response(200, json={"request_id": "upstream-request", "client_id": "org123", "domain": "cybersecurity", "result": {"ok": True}}))
    request_id = "12345678-1234-4234-8234-123456789012"
    response = client.post("/v1/cybersecurity/execute", headers={**AUTH_HEADERS, "X-Request-ID": request_id}, json={"tenant_id": "org123", "payload": {"case_id": "E2E-cybersecurity", "alert": "execute this"}})
    assert response.status_code == 200
    assert route.called
...
def test_execute_rejects_invalid_tenant_id(monkeypatch):
    configure_static_test_auth(monkeypatch)
    response = client.post("/v1/cybersecurity/execute", headers=AUTH_HEADERS, json={"tenant_id": "ORG_BAD", "payload": BASE_PAYLOAD})
    assert response.status_code == 422
...
def test_execute_caches_oauth_token_across_requests(monkeypatch):
    ...
    token_route = respx.post("https://idp.internal/oauth/token").mock(return_value=Response(200, json={"access_token": "minted-token", "expires_in": 300}))
    upstream_route = respx.post("https://fde-mastery.internal/v1/finance/execute").mock(return_value=Response(200, json={"request_id": "upstream-request", "client_id": "org123", "domain": "finance", "result": {"ok": True}}))
    for index in range(2):
        response = client.post("/v1/finance/execute", headers={"Authorization": "Bearer secret", "Idempotency-Key": f"oauth-test-{index}"}, json={"tenant_id": "org123", "payload": {"case_id": f"OAUTH-{index}", "synthetic": True}})
        assert response.status_code == 200
