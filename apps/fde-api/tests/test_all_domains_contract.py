...
def test_gateway_forwards_every_supported_domain_to_canonical_triage(monkeypatch):
    ...
    routes = {
        domain: respx.post(f"https://fde-mastery.internal/v1/triage/org123/{domain}").mock(
            return_value=Response(200, json={"request_id": f"upstream-{domain}", "client_id": "org123", "domain": domain, "result": {"domain": domain}, "confidence": 0.9, "processing_time_ms": 1.2, "audit_log_id": f"AUDIT-{domain}"})
        )
        for domain in DOMAINS
    }
    client = TestClient(app)
    for index, domain in enumerate(DOMAINS):
        response = client.post(f"/v1/{domain}/execute", headers={"Authorization": "Bearer secret", "Idempotency-Key": f"domain-contract-{domain}", "X-Request-ID": f"12345678-1234-4234-8234-{index:012d}"}, json={"tenant_id": "org123", "payload": PAYLOADS[domain]})
...
def test_unknown_domain_fails_closed(monkeypatch):
    ...
    monkeypatch.setenv("FDE_MASTER_UPSTREAM_URL", "https://fde-mastery.internal")
    monkeypatch.setenv("FDE_MASTER_UPSTREAM_TOKEN", "static-token")
    response = TestClient(app).post("/v1/unknown/execute", headers={"Authorization": "Bearer secret", "Idempotency-Key": "unknown-domain"}, json={"tenant_id": "org123", "payload": {"synthetic": True}})
