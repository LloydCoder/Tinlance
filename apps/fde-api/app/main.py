...
VALID_DOMAINS = {
    "cybersecurity", "finance", "healthtech", "logistics", "legal", "revops", "procurement", "custom",
}

app = FastAPI(title="Tinlance FDE API", version="0.5.1", docs_url=("/docs" if os.getenv("FDE_ENABLE_DOCS", "false").lower() == "true" else None), redoc_url=None)
allowed_hosts = [host.strip() for host in os.getenv("FDE_ALLOWED_HOSTS", "localhost,127.0.0.1,testserver").split(",") if host.strip()]
app.add_middleware(TrustedHostMiddleware, allowed_hosts=allowed_hosts)

class ExecutionRequest(BaseModel):
    model_config = ConfigDict(extra="forbid")
    tenant_id: str = Field(min_length=1, max_length=100, pattern=r"^[a-z0-9-]+$")
...
def static_token_allowed() -> bool:
    return os.getenv("FDE_ENV", "production").strip().lower() in {"development", "test"}
...
async def get_upstream_token() -> str | None:
    ...
    if token_url and client_id and client_secret:
        ...
        form: dict[str, str] = {"grant_type": "client_credentials", "client_id": client_id, "client_secret": client_secret}
        if audience:
...
async def ready() -> dict[str, str]:
    has_oauth = bool(os.getenv("FDE_OAUTH_TOKEN_URL") and os.getenv("FDE_OAUTH_CLIENT_ID") and os.getenv("FDE_OAUTH_CLIENT_SECRET"))
    has_static_auth = static_token_allowed() and bool(os.getenv("FDE_MASTER_UPSTREAM_TOKEN"))
    ...
    if not (has_static_auth or has_oauth):
        raise HTTPException(status_code=503, detail="Production OAuth upstream authentication is required")
    return {"status": "ready"}

@app.post("/v1/{domain}/execute", response_model=ExecutionResponse, dependencies=[Depends(require_service_token)])
async def execute(domain: str, payload: ExecutionRequest, request: Request, idempotency_key: Annotated[str | None, Header(alias="Idempotency-Key")] = None) -> ExecutionResponse:
    ...
    if not token:
        raise HTTPException(status_code=503, detail="Production OAuth upstream authentication is required")
    headers = {"x-request-id": request_id, "Idempotency-Key": idempotency_key, "authorization": f"Bearer {token}"}
    try:
        async with httpx.AsyncClient(timeout=httpx.Timeout(30.0, connect=5.0)) as client:
            response = await client.post(upstream.rstrip("/") + f"/v1/{normalized_domain}/execute", json={"tenant_id": payload.tenant_id, "payload": payload.payload}, headers=headers)
    ...
    return ExecutionResponse(request_id=request_id, status="completed", upstream_status=response.status_code, result=result)
