...
function keyMaterial() {
  ...
  if (!configured) throw new Error("WEBHOOK_SECRET_ENCRYPTION_KEY is not configured");
...
export function signWebhook(secret: string, timestamp: number, body: string) { const digest = createHash("sha256").update(`${timestamp}.${body}`).digest("hex"); return `sha256=${digest}`; }
