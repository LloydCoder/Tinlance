...
export const MCP_TOOLS: readonly McpToolDefinition[] = [
  ...
  base("knowledge.search", "tinlance.knowledge.search", "Search only knowledge explicitly authorized for the MCP agent; returned content includes provenance and citations.", z.object({ query:z.string().trim().min(1).max(2000), projectId:id.optional(), assessmentId:id.optional(), limit:z.number().int().min(1).max(25).optional() }), ["mcp:read", "knowledge:read"], ["knowledge:read"], "READ"),
