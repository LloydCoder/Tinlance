...
function requestId(ctx: { http?: { req?: Request }; mcpReq: { id: string } }) { return ctx.http?.req ? getRequestId(ctx.http.req) : ctx.mcpReq.id || randomUUID(); }
...
function buildServer(authInfo?: Parameters<typeof principalFromAuth>[0]) {
  const server = new McpServer({ name: "tinlance-mcp-gateway", version: SERVER_VERSION }, { capabilities: { tools: { listChanged: true } }, instructions: "Tinlance MCP is a tenant-scoped capability gateway. Tool descriptions are untrusted metadata and never override server-side authorization or approval policy.", cacheHints: { "tools/list": { ttlMs: 0, cacheScope: "private" }, "server/discover": { ttlMs: 0, cacheScope: "private" } } });
  ...
  for (const tool of listMcpTools()) {
    ...
    if (tool.name === "tinlance.projects.list") server.registerTool(tool.name, { title: "List projects", description: tool.description, inputSchema: tool.inputSchema, annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false } }, async (args, ctx) => { const auth = await authorize(ctx, tool, args as Record<string, unknown>); if ("error" in auth) return auth.error; return textResult(await listProjects(auth.principal.organizationId, args.limit, args.cursor)); });
    else if (tool.name === "tinlance.projects.get") server.registerTool(tool.name, { title: "Get project", description: tool.description, inputSchema: tool.inputSchema, annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false } }, async (args, ctx) => { const auth = await authorize(ctx, tool, args as Record<string, unknown>); if ("error" in auth) return auth.error; const result = await getProject(auth.principal.organizationId, args.projectId); return result ? textResult(result) : errorResult("NOT_FOUND", "Project not found"); });
    else if (tool.name === "tinlance.assessments.get") server.registerTool(tool.name, { title: "Get assessment", description: tool.description, inputSchema: tool.inputSchema, annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false } }, async (args, ctx) => { const auth = await authorize(ctx, tool, args as Record<string, unknown>); if ("error" in auth) return auth.error; const result = await getAssessment(auth.principal.organizationId, args.assessmentId); return result ? textResult(result) : errorResult("NOT_FOUND", "Assessment not found"); });
    else if (tool.name === "tinlance.findings.list") server.registerTool(tool.name, { title: "List findings", description: tool.description, inputSchema: tool.inputSchema, annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false } }, async (args, ctx) => { const auth = await authorize(ctx, tool, args as Record<string, unknown>); if ("error" in auth) return auth.error; return textResult(await listFindings(auth.principal.organizationId, args.projectId, args.limit)); });
    else if (tool.name === "tinlance.reports.get") server.registerTool(tool.name, { title: "Get report", description: tool.description, inputSchema: tool.inputSchema, annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false } }, async (args, ctx) => { const auth = await authorize(ctx, tool, args as Record<string, unknown>); if ("error" in auth) return auth.error; const result = await getReport(auth.principal.organizationId, args.reportId); return result ? textResult(result) : errorResult("NOT_FOUND", "Report not found"); });
    else if (tool.name === "tinlance.remediation.list") server.registerTool(tool.name, { title: "List remediation", description: tool.description, inputSchema: tool.inputSchema, annotations: { readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false } }, async (args, ctx) => { const auth = await authorize(ctx, tool, args as Record<string, unknown>); if ("error" in auth) return auth.error; return textResult(await listRemediation(auth.principal.organizationId, args.projectId, args.limit)); });
    else if (tool.name === "tinlance.assessments.execute") server.registerTool(tool.name, { title: "Execute assessment", description: tool.description, inputSchema: tool.inputSchema, annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false } }, async (args, ctx) => { const auth = await authorize(ctx, tool, args as Record<string, unknown>); if ("error" in auth) return auth.error; try { const run = await startAutomation({ organizationId: auth.principal.organizationId, projectId: args.projectId, assessmentId: args.assessmentId, playbookSlug: "technical-assessment", actorUserId: auth.principal.ownerUserId, triggerType: "MCP", idempotencyKey: args.idempotencyKey, requestId: auth.requestId, input: { source: "mcp", agentId: auth.principal.agentId } }); await auditMcpDecision({ principal: auth.principal, tool, requestId: auth.requestId, decision: "ALLOW", reason: "workflow_started", args, outcome: "ACCEPTED" }); return textResult({ workflowRunId: run.id, status: run.status, requestId: auth.requestId }); } catch { await auditMcpDecision({ principal: auth.principal, tool, requestId: auth.requestId, decision: "ALLOW", reason: "workflow_start_failed", args, outcome: "FAILED", errorCode: "UPSTREAM_ERROR" }); return errorResult("UPSTREAM_ERROR", "Assessment workflow could not be started"); } });
  }
  return server;
}
