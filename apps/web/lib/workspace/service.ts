...
export async function ensureProjectWorkspaceState(projectId: string, organizationId: string) {
  ...
  const existing = await db.projectWorkspaceState.findUnique({ where: { projectId }, select: { id: true, organizationId: true } });
...
export async function transitionProject(input: { projectId: string; organizationId: string; actorUserId: string; to: ProjectWorkspaceStatus; requestId: string }) {
  ...
  return db.$transaction(async (tx) => {
    const result = await tx.projectWorkspaceState.updateMany({ where: { id: state.id, organizationId: input.organizationId, status: state.status }, data: { status: input.to, completedAt: input.to === "COMPLETED" ? new Date() : state.completedAt } });
...
export async function transitionFinding(input: { findingId: string; organizationId: string; actorUserId: string; to: WorkspaceFindingStatus; requestId: string }) {
  const finding = await db.workspaceFinding.findFirst({ where: { id: input.findingId, organizationId: input.organizationId }, select: { id: true, status: true, projectId: true } });
  if (!finding) throw new Error("finding_not_found");
  assertFindingTransition(finding.status, input.to);
  return db.$transaction(async (tx) => {
    const result = await tx.workspaceFinding.updateMany({ where: { id: finding.id, organizationId: input.organizationId, status: finding.status }, data: { status: input.to, visibility: ["CUSTOMER_VISIBLE", "OPEN", "ACKNOWLEDGED", "IN_PROGRESS", "REMEDIATED", "VERIFICATION_PENDING", "VERIFIED", "CLOSED"].includes(input.to) ? "CUSTOMER" : undefined } });
...
export async function transitionRemediation(input: { remediationId: string; organizationId: string; actorUserId: string; to: WorkspaceRemediationStatus; requestId: string }) {
  const remediation = await db.workspaceRemediation.findFirst({ where: { id: input.remediationId, organizationId: input.organizationId }, select: { id: true, status: true, projectId: true, findingId: true } });
  if (!remediation) throw new Error("remediation_not_found");
  assertRemediationTransition(remediation.status, input.to);
  return db.$transaction(async (tx) => {
    const result = await tx.workspaceRemediation.updateMany({ where: { id: remediation.id, organizationId: input.organizationId, status: remediation.status }, data: { status: input.to, completedAt: ["VERIFIED", "CLOSED"].includes(input.to) ? new Date() : undefined, verificationStatus: input.to === "VERIFIED" ? "PASS" : undefined } });
    if (result.count !== 1) throw new Error("remediation_transition_race");
    if (input.to === "READY_FOR_VERIFICATION") {
      const finding = await tx.workspaceFinding.findFirst({ where: { id: remediation.findingId, organizationId: input.organizationId }, select: { status: true } });
      if (finding && ["IN_PROGRESS", "REMEDIATED"].includes(finding.status)) await tx.workspaceFinding.updateMany({ where: { id: remediation.findingId, organizationId: input.organizationId }, data: { status: "VERIFICATION_PENDING", visibility: "CUSTOMER" } });
    }
...
export async function transitionReport(input: { reportId: string; organizationId: string; actorUserId: string; to: WorkspaceReportStatus; requestId: string }) {
  const report = await db.workspaceReport.findFirst({ where: { id: input.reportId, organizationId: input.organizationId }, select: { id: true, status: true, currentVersion: true } });
  if (!report) throw new Error("report_not_found");
  assertReportTransition(report.status, input.to);
  return db.$transaction(async (tx) => {
    const result = await tx.workspaceReport.updateMany({ where: { id: report.id, organizationId: input.organizationId, status: report.status }, data: { status: input.to, publishedAt: input.to === "PUBLISHED" ? new Date() : undefined, publishedByUserId: input.to === "PUBLISHED" ? input.actorUserId : undefined } });
