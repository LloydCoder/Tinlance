...
export function WorkflowClient({ projectId, playbooks, initialRuns, assessments }: { projectId: string; playbooks: Playbook[]; initialRuns: Run[]; assessments: Assessment[] }) {
  ...
  async function start(slug: string) {
    if (!assessmentId) { setMessage("Create or scope an assessment before starting the FDE Technical Assessment playbook."); return; }
    setBusy(slug); setMessage("");
    try {
      const response = await fetch(`/api/v1/automation/playbooks/${encodeURIComponent(slug)}/runs`, { method: "POST", headers: { "content-type": "application/json", "Idempotency-Key": `${projectId}:${slug}:${assessmentId}:${Date.now()}` }, body: JSON.stringify({ projectId, assessmentId, scope: { projectId, assessmentId }, context: { customerControlledContentIsUntrusted: true } }) });
