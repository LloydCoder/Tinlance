...
export default async function OpportunitiesPage() {
  ...
  const rows = opportunities.map((opportunity) => ({
    ...
    meta: `${opportunity.nextAction ?? "No next action"}${opportunity.nextActionAt ? ` · ${opportunity.nextActionAt.toLocaleDateString()}` : ""}`,
  }));
  return <AdminResourcePage active="opportunities" kicker="COMMERCIAL ENGINE / OPPORTUNITIES" title="Commercial pipeline." description="Every active opportunity has a controlled stage, owner context and next action. Stale work is surfaced by explicit next-action dates rather than disappearing into a spreadsheet." columns={["Opportunity", "Need / contact", "Activity", "Stage"]} rows={rows} />;
}
