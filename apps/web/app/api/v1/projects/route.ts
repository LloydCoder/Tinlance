...
import { authenticateApi, cursorPage, emitApiEvent, ok, parsePagination, problem } from "@/lib/api/v1";
...
export async function POST(request: Request) {
  const auth = await authenticateApi(request, "projects:write"); if ("response" in auth) return auth.response; const { principal } = auth; let body; try { body = createSchema.parse(await request.json()); } catch { return problem(principal.requestId, 422, "validation_failed", "Invalid project request"); }
