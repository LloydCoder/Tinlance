...
export async function POST(request: Request, context: { params: Promise<{ projectId: string }> }) {
  ...
  const body = await request.json().catch(() => ({})) as { assessmentId?: string; title?: string; type?: string; idempotencyKey?: string };
  const workspaceAssessmentId = body.assessmentId?.trim(); const title = body.title?.trim(); const type = body.type?.trim() || "Technical Assessment"; const idempotencyKey = body.idempotencyKey?.trim() || request.headers.get("idempotency-key")?.trim() || null;
  if (!workspaceAssessmentId || !title || title.length > 240) return NextResponse.json({ error: "invalid_request", requestId }, { status: 400, headers: { "cache-control": "no-store", "x-request-id": requestId } });
  const assessment = await db.workspaceAssessment.findFirst({ where: { id: workspaceAssessmentId, projectId, organizationId: authorized.project.organizationId }, select: { id: true, assessmentId: true, type: true, objective: true, methodology: true, startedAt: true, completedAt: true } });
  ...
  if (idempotencyKey) { const existing = await db.workspaceReport.findUnique({ where: { idempotencyKey }, select: { id: true, projectId: true, organizationId: true, currentVersion: true } }); if (existing && existing.projectId === projectId && existing.organizationId === authorized.project.organizationId) return NextResponse.json({ status: "duplicate", reportId: existing.id, version: existing.currentVersion, requestId }, { headers: { "cache-control": "no-store", "x-request-id": requestId } }); if (existing) return NextResponse.json({ error: "idempotency_conflict", requestId }, { status: 409, headers: { "cache-control": "no-store", "x-request-id": requestId } }); }
  const findings = await db.workspaceFinding.findMany({ where: { assessmentId: assessment.assessmentId, projectId, organizationId: authorized.project.organizationId, visibility: { in: ["CUSTOMER","CUSTOMER_CONFIDENTIAL"] }, status: { notIn: ["DRAFT_INTERNAL","REVIEW"] } }, orderBy: [{ severity: "asc" }, { createdAt: "asc" }], select: { id: true, title: true, description: true, severity: true, category: true, affectedAsset: true, recommendation: true } });
  ...
  try {
    const report = await db.$transaction(async (tx) => {
      const created = await tx.workspaceReport.create({ data: { organizationId: authorized.project.organizationId, projectId, assessmentId: assessment.assessmentId, title, type, status: "DRAFT", currentVersion: 1, summary: `Evidence-backed report containing ${findings.length} customer-visible findings and ${evidence.length} verified evidence items.`, generatedAt: new Date(), contentHash: generated.contentHash, idempotencyKey } });
  ...
  } catch (error) { console.error("workspace_report_create_failed", { requestId, projectId, error }); return NextResponse.json({ error: "service_unavailable", requestId }, { status: 503, headers: { "cache-control": "no-store", "x-request-id": requestId } }); }
}
