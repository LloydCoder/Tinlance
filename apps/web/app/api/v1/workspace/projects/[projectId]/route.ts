...
export async function GET(request: Request, context: { params: Promise<{ projectId: string }> }) {
  ...
  const [assessments, findings, remediation, reports, evidenceRequests] = await Promise.all([
  ...
  const activityResources = [
    ...
    ...assessments.map((item) => ({ resourceType: "workspace_assessment", resourceId: item.id })),
    ...findings.map((item) => ({ resourceType: "workspace_finding", resourceId: item.id })),
    ...remediation.map((item) => ({ resourceType: "workspace_remediation", resourceId: item.id })),
    ...reports.map((item) => ({ resourceType: "workspace_report", resourceId: item.id })),
    ...evidenceRequests.map((item) => ({ resourceType: "workspace_evidence_request", resourceId: item.id })),
  ];
  const recentActivity = activityResources.length ? await db.auditEvent.findMany({ where: { organizationId: authorized.project.organizationId, OR: activityResources, orderBy: { createdAt: "desc" }, take: 25, select: { id: true, action: true, resourceType: true, resourceId: true, actorUserId: true, createdAt: true, metadata: true } }) : [];
