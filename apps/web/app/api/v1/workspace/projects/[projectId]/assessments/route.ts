...
const schema = z.object({
  ...
  type: z.enum(["cybersecurity","finance","healthtech","logistics","legal","revops","procurement","custom"]),
  objective: z.string().trim().min(10).max(5000),
  scope: z.record(z.string(), z.unknown()),
  methodology: z.string().trim().min(10).max(10000),
...
export async function POST(request: Request, context: { params: Promise<{ projectId: string }> }) {
  ...
  try {
    ...
    console.error("workspace_assessment_create_failed", { requestId, projectId, error });
