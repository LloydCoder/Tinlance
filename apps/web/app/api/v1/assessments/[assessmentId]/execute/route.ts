...
export async function POST(request: Request, context: { params: Promise<{ assessmentId: string }> }) {
  ...
  const assessment = await db.workspaceAssessment.findFirst({ where: { id: assessmentId, organizationId: auth.principal.organizationId }, select: { id: true, projectId: true, organizationId: true, type: true, status: true } });
  ...
  try {
    const run = await startAutomation({ organizationId: auth.principal.organizationId, projectId: assessment.projectId, playbookSlug: "fde-technical-assessment", actorUserId: auth.principal.userId, triggerType: "API", idempotencyKey: `api-assessment:${assessmentId}:${idempotencyKey}`, requestId: auth.principal.requestId, input: { assessmentId, domain, source: "api-v1" } });
