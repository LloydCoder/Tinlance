...
import { auditCommercialTransition, createEntitlement, createOnboarding, newId, recordOutboxEvent, requestProvisioning } from "@/lib/platform/lifecycle";
...
export async function POST(request: Request) {
  ...
  try { body = JSON.parse(payload) as PaystackPayload; } catch { body = null; }
  ...
  try {
    await db.$transaction(async (tx) => {
      try {
        await tx.webhookEvent.create({ data: { provider: "paystack", eventId, eventType } });
      } catch (error) {
        if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") throw new Error("duplicate_webhook");
        throw error;
      }

      if (!nextStatus || !reference) {
        await tx.auditEvent.create({ data: { action: `paystack.${eventType}`, resourceType: "webhook", resourceId: eventId, requestId, metadata: { reference, eventType } } });
        return;
      }
      ...
      if (invoices.length !== 1) {
        await tx.auditEvent.create({ data: { action: `paystack.${eventType}.unmatched`, resourceType: "webhook", resourceId: eventId, requestId, metadata: { reference, eventType, matchCount: invoices.length } } });
        return;
      }
      ...
      if (!canTransition(invoice.status, nextStatus)) {
        await tx.auditEvent.create({ data: { organizationId: invoice.organizationId, action: `paystack.${eventType}.invalid_transition`, resourceType: "invoice", resourceId: invoice.id, requestId, metadata: { eventId, reference, eventType, previousStatus: invoice.status, nextStatus } } });
        return;
      }
      ...
      if (!paymentId) {
        paymentId = newId();
        await tx.$executeRaw`
          INSERT INTO "Payment" ("id", "invoiceId", "organizationId", "provider", "providerReference", "providerTransactionId", "status", "amountMinor", "currency", "lastProviderEventId")
          VALUES (${paymentId}, ${invoice.id}, ${invoice.organizationId}, 'paystack', ${reference}, ${providerTransactionId}, ${nextStatus === "paid" ? "SUCCEEDED" : nextStatus === "refunded" ? "REFUNDED" : "FAILED"}, ${invoice.amountMinor}, ${invoice.currency}, ${eventId})
          ON CONFLICT ("providerReference") DO UPDATE SET "providerTransactionId" = EXCLUDED."providerTransactionId", "status" = EXCLUDED."status", "lastProviderEventId" = EXCLUDED."lastProviderEventId", "updatedAt" = NOW()
        `;
      } else {
        const paymentStatus = nextStatus === "paid" ? "SUCCEEDED" : nextStatus === "refunded" ? "REFUNDED" : "FAILED";
        await tx.$executeRaw`UPDATE "Payment" SET "status" = ${paymentStatus}, "providerTransactionId" = COALESCE(${providerTransactionId}, "providerTransactionId"), "lastProviderEventId" = ${eventId}, "updatedAt" = NOW() WHERE "id" = ${paymentId}`;
      }
      ...
      if (nextStatus === "paid") {
        const commercial = await tx.$queryRaw<Array<{ proposalId: string | null; title: string; pricing: unknown }>>`
          SELECT i."proposalId", p."title", pv."pricing" FROM "Invoice" i
          LEFT JOIN "Proposal" p ON p."id" = i."proposalId"
          LEFT JOIN "ProposalVersion" pv ON pv."proposalId" = p."id" AND pv."version" = p."currentVersion"
        ...
        await tx.$executeRaw`
          INSERT INTO "Entitlement" ("id", "organizationId", "product", "capability", "sourceType", "sourceId", "proposalId", "invoiceId", "paymentId", "status")
          VALUES (${entitlementId}, ${invoice.organizationId}, ${product}, ${capability}, 'PAYMENT', ${paymentId}, ${proposalId}, ${invoice.id}, ${paymentId}, 'ACTIVE')
          ON CONFLICT ("organizationId", "product", "capability", "sourceType", "sourceId") DO UPDATE SET "status" = 'ACTIVE', "paymentId" = EXCLUDED."paymentId", "updatedAt" = NOW()
        `;
