  ...
  try {
    await db.$transaction(async (tx) => {
      ...
      if (nextStatus === "paid") {
        ...
        await recordOutboxEvent({ eventKey: `email.payment.succeeded:${invoice.id}`, eventType: "email.payment.succeeded", aggregateType: "invoice", aggregateId: invoice.id, organizationId: invoice.organizationId, payload: { recipient: "", invoiceId: invoice.id }, tx });
