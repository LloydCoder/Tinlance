...
function pricingValue(pricing: Prisma.JsonValue, key: string) {
  ...
  const value = (pricing as Record<string, Prisma.JsonValue>)[key];
  return value ?? null;
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const proposal = await db.proposal.findUnique({
      ...
      include: { lead: true, versions: { where: { version: undefined }, orderBy: { version: "desc" }, take: 1 } },
    });
    ...
    if (proposal.status === ProposalStatus.ACCEPTED) {
      const invoice = await db.invoice.findFirst({ where: { organizationId: proposal.organizationId ?? "", description: { contains: proposal.proposalNumber } }, select: { id: true, status: true } });
    ...
    if (proposal.status !== ProposalStatus.SENT && proposal.status !== ProposalStatus.VIEWED) {
      return NextResponse.json({ error: "proposal_not_accepting", requestId }, { status: 409, headers: { "cache-control": "no-store", "x-request-id": requestId } });
    }
    ...
    const totalMinor = pricingValue(version.pricing, "totalMinor");
    const currencyValue = pricingValue(version.pricing, "currency");
    const currency = typeof currencyValue === "string" ? currencyValue.toUpperCase() : "USD";
    if (typeof totalMinor !== "number" || !Number.isSafeInteger(totalMinor) || totalMinor <= 0) {
      return NextResponse.json({ error: "proposal_not_payable", requestId }, { status: 409, headers: { "cache-control": "no-store", "x-request-id": requestId } });
    }

    const { token: paymentToken, hash: paymentHash } = newCommercialToken();
    const paymentTokenExpiresAt = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000);
    const paymentReference = newPaymentReference(proposal.id);
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      if (!organizationId) {
        ...
        if (!organizationId) {
          organizationId = (await tx.organization.create({ data: { name: proposal.lead.organizationName, slug: slugify(proposal.lead.organizationName), websiteDomain: domain }, select: { id: true } })).id;
        }
      }

      const existingInvoice = await tx.invoice.findFirst({ where: { externalId: paymentReference }, select: { id: true, status: true, amountMinor: true, currency: true } });
      if (existingInvoice) return { organizationId, invoiceId: existingInvoice.id, invoiceStatus: existingInvoice.status, paymentToken, paymentCreated: false };
      ...
      const invoice = await tx.invoice.create({
        data: {
          ...
          description: `Proposal ${proposal.proposalNumber} — ${proposal.title}`,
          status: "sent",
          externalId: paymentReference,
          paymentAccessRequired: true,
          dueAt: proposal.expiresAt ?? new Date(Date.now() + 30 * 24 * 60 * 60 * 1000),
        },
        select: { id: true, status: true },
      });

      await tx.$executeRaw`
        INSERT INTO "CommercialToken" ("id", "kind", "entityId", "tokenHash", "expiresAt")
        VALUES (${newId()}, 'payment', ${invoice.id}, ${paymentHash}, ${paymentTokenExpiresAt})
      `;
      ...
      await tx.proposal.update({
        ...
        data: {
          ...
          acceptedByEmail: parsed.data.acceptedByEmail,
          organizationId,
        },
      });
      ...
      if (proposal.opportunityId) {
        await tx.opportunity.update({
          ...
          data: { organizationId, stage: OpportunityStage.ENGAGEMENT_PENDING, lastActivityAt: new Date(), nextAction: "Collect payment", nextActionAt: new Date(Date.now() + 24 * 60 * 60 * 1000) },
        });
      }
      ...
      await auditCommercialTransition({ organizationId, action: "invoice.created", resourceType: "invoice", resourceId: invoice.id, requestId, newState: invoice.status, metadata: { proposalId: proposal.id, amountMinor: totalMinor, currency }, tx });
      await recordOutboxEvent({ eventKey: `proposal.accepted:${proposal.id}:${proposal.currentVersion}`, eventType: "proposal.accepted", aggregateType: "proposal", aggregateId: proposal.id, organizationId, payload: { proposalId: proposal.id, version: proposal.currentVersion, invoiceId: invoice.id }, tx });
      await recordOutboxEvent({ eventKey: `invoice.created:${invoice.id}`, eventType: "invoice.created", aggregateType: "invoice", aggregateId: invoice.id, organizationId, payload: { invoiceId: invoice.id, proposalId: proposal.id }, tx });

      return { organizationId, invoiceId: invoice.id, invoiceStatus: invoice.status, paymentToken, paymentCreated: true };
    });
    ...
    try {
      ...
      console.error("growth_event_record_failed", { requestId, error });
    }
    ...
    const email = await sendCommercialEmail({
      ...
      html: `<p>Thank you. Your proposal <strong>${proposal.proposalNumber}</strong> has been accepted.</p><p>Invoice <strong>${result.invoiceId}</strong> is ready for payment.</p><p><a href="${paymentUrl}">Continue to secure payment</a>.</p>`,
      requestId,
      organizationId: result.organizationId,
    }).catch((error) => {
      console.error("acceptance_notification_failed", { requestId, error });
