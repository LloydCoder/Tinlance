...
export async function POST(request: Request) {
  ...
  try {
    ...
    const version = proposal.versions[0]; if (!version) return NextResponse.json({ error: "proposal_version_missing", requestId }, { status: 409, headers: { "cache-control": "no-store", "x-request-id": requestId } });
    const { token: onboardingToken, hash: onboardingHash } = createProposalToken(); const onboardingExpiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
    const result = await db.$transaction(async (tx) => {
      ...
      await tx.auditEvent.createMany({ data: [{ organizationId, action: "proposal.accepted", resourceType: "proposal", resourceId: proposal.id, requestId, metadata: { acceptedByEmail: parsed.data.acceptedByEmail, version: proposal.currentVersion } }, { organizationId, action: "engagement.created", resourceType: "engagement", resourceId: created.id, requestId, metadata: { proposalId: proposal.id, deliveryModel: DeliveryModel.PROJECT, commercialValueMinor: totalMinor, currency } }, { organizationId, action: "client.onboarding_initiated", resourceType: "client", resourceId: client.id, requestId, metadata: { email: proposal.lead.email, expiresAt: onboardingExpiresAt.toISOString() } }] });
      return { ...created, onboardingToken, onboardingCreated: true };
    });
    try { await recordGrowthEvent({ eventName: "proposal_accepted", source: "proposal", entityId: proposal.id, privacyClass: "FINANCIAL", properties: { engagementId: result.id } }); await recordGrowthEvent({ eventName: "deal_won", source: "proposal", entityId: proposal.id, privacyClass: "FINANCIAL" }); } catch (error) { console.error("growth_event_record_failed", { requestId, error }); }
    ...
    console.error("proposal_acceptance_failed", { requestId, error }); return NextResponse.json({ error: "service_unavailable", requestId }, { status: 503, headers: { "cache-control": "no-store", "x-request-id": requestId } });
  }
}
