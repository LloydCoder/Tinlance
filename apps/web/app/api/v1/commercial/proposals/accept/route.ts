  ...
  try {
    ...
    const engagement = await db.$transaction(async (tx) => {
      ...
      if (!organizationId) {
        ...
        if (!organizationId) {
          const createdOrg = await tx.organization.create({ data: { name: proposal.lead.organizationName, slug: slugify(proposal.lead.organizationName), websiteDomain: domain } });
      ...
      const existingEngagement = await tx.engagement.findUnique({ where: { proposalId: proposal.id }, select: { id: true } });
      if (existingEngagement) return existingEngagement;
      ...
      const created = await tx.engagement.create({ data: {
        ...
        commercialValueMinor: totalMinor, currency, deliveryModel: DeliveryModel.PROJECT, status: EngagementStatus.ACTIVE,
        securityRequirements: version.securityConsiderations ?? null,
    ...
    const email = await sendCommercialEmail({ action: "proposal.accepted", resourceId: proposal.id, to: proposal.lead.email, subject: `Tinlance engagement created — ${proposal.title}`, html: `<p>Thank you. Your proposal <strong>${proposal.proposalNumber}</strong> has been accepted and the Tinlance engagement workspace is being prepared.</p>`, requestId, organizationId: engagement.organizationId }).catch((error) => { console.error("acceptance_notification_failed", { requestId, error }); return { sent: false, duplicate: false, configured: false }; });
    return NextResponse.json({ status: "accepted", requestId, engagementId: engagement.id, organizationId: engagement.organizationId, clientId: engagement.clientId, notification: email }, { status: 201, headers: { "cache-control": "no-store", "x-request-id": requestId } });
  } catch (error) {
