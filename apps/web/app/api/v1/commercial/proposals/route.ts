...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (parsed.data.assessmentId) { const assessment = await db.assessment.findUnique({ where: { id: parsed.data.assessmentId }, select: { leadId: true } }); if (!assessment || assessment.leadId !== lead.id) return NextResponse.json({ error: "assessment_mismatch", requestId }, { status: 400, headers: { "cache-control": "no-store", "x-request-id": requestId } }); }
    if (parsed.data.opportunityId) { const opportunity = await db.opportunity.findUnique({ where: { id: parsed.data.opportunityId }, select: { leadId: true } }); if (!opportunity || opportunity.leadId !== lead.id) return NextResponse.json({ error: "opportunity_mismatch", requestId }, { status: 400, headers: { "cache-control": "no-store", "x-request-id": requestId } }); }
    ...
    const proposal = await db.$transaction(async (tx) => {
      const created = await tx.proposal.create({ data: { proposalNumber, organizationId: lead.organizationId, leadId: lead.id, assessmentId: parsed.data.assessmentId, opportunityId: parsed.data.opportunityId, title: parsed.data.title, publicTokenHash: hash, expiresAt, createdByUserId: actor.userId, versions: { create: { version: 1, ...parsed.data.version, pricing: parsed.data.version.pricing, createdByUserId: actor.userId } } }, select: { id: true, proposalNumber: true } });
    ...
    try { await recordGrowthEvent({ eventName: "proposal_created", source: "crm", organizationId: lead.organizationId ?? undefined, entityId: proposal.id, privacyClass: "INTERNAL", properties: { opportunityId: parsed.data.opportunityId ?? null } }); } catch (error) { console.error("growth_event_record_failed", { requestId, eventName: "proposal_created", error }); }
  ...
  } catch (error) { console.error("proposal_creation_failed", { requestId, error }); return NextResponse.json({ error: "service_unavailable", requestId }, { status: 503, headers: { "cache-control": "no-store", "x-request-id": requestId } }); }
}
