...
export async function POST(request: Request) {
  ...
  try {
    ...
    const qualification = qualifyAssessment(parsed.data);
    ...
    try {
      result = await db.$transaction(async (tx) => {
        ...
        const lead = await tx.lead.create({ data: {
          ...
          country: parsed.data.country, service: parsed.data.capability, source: parsed.data.source, campaign: parsed.data.campaign,
          referral: parsed.data.referral, roleTitle: parsed.data.roleTitle, companySize: parsed.data.companySize, website: parsed.data.website || null,
          problemStatement: parsed.data.problem, desiredOutcome: parsed.data.desiredOutcome, urgency: parsed.data.urgency,
          budgetSignal: parsed.data.budgetSignal, timeline: parsed.data.timeline, technicalEnvironment: parsed.data.technicalEnvironment,
          securitySensitivity: parsed.data.securitySensitivity, qualificationScore: qualification.score,
          qualificationStatus: qualification.status as QualificationStatus, nextAction: qualification.nextAction, nextActionAt, consent: parsed.data.consent,
          notes: parsed.data.constraints, idempotencyKey,
        }, select: { id: true } });

        const assessment = await tx.assessment.create({ data: {
          leadId: lead.id, problem: parsed.data.problem, workflow: parsed.data.workflow, currentArchitecture: parsed.data.currentArchitecture,
          constraints: parsed.data.constraints, desiredOutcome: parsed.data.desiredOutcome, urgency: parsed.data.urgency,
          stakeholders: parsed.data.stakeholders, existingSystems: parsed.data.existingSystems, securityRequirements: parsed.data.securityRequirements,
          businessImpact: parsed.data.businessImpact, idempotencyKey,
        }, select: { id: true } });

        const opportunity = await tx.opportunity.create({ data: {
          leadId: lead.id, assessmentId: assessment.id, stage: qualification.status === "QUALIFIED" ? OpportunityStage.QUALIFIED : OpportunityStage.QUALIFYING,
          currency: "USD", nextAction: qualification.nextAction, nextActionAt, lastActivityAt: new Date(),
        }, select: { id: true } });
    ...
    console.error("assessment_submission_failed", { requestId, error });
