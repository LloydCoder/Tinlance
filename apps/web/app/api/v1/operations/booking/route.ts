...
export async function POST(request: Request) {
  ...
  try {
    ...
    let raw: unknown;
    try { raw = JSON.parse(bodyText); } catch { return NextResponse.json({ error: "invalid_json", requestId }, { status: 400, headers: { "cache-control": "no-store", "x-request-id": requestId } }); }
    ...
    if (!Number.isFinite(startsAt.getTime()) || startsAt.getTime() <= Date.now()) return NextResponse.json({ error: "invalid_start_time", requestId }, { status: 400, headers: { "cache-control": "no-store", "x-request-id": requestId } });

    let result: { id: string; duplicate: boolean; opportunityId: string | null };
    try {
      result = await db.$transaction(async (tx) => {
        if (idempotencyKey) {
          const existing = await tx.booking.findUnique({ where: { idempotencyKey }, select: { id: true, opportunityId: true } });
          if (existing) return { id: existing.id, duplicate: true, opportunityId: existing.opportunityId };
        }
        ...
        if (assessmentId) {
          ...
          opportunityId = assessment.opportunity?.id ?? opportunityId;
        }
        if (leadId) {
          const lead = await tx.lead.findUnique({ where: { id: leadId }, select: { id: true, email: true } });
          if (!lead || lead.email.toLowerCase() !== parsed.data.email.toLowerCase()) throw new Error("booking_lead_mismatch");
        }
        if (opportunityId) {
          const opportunity = await tx.opportunity.findUnique({ where: { id: opportunityId }, select: { id: true, leadId: true, assessmentId: true, stage: true } });
          if (!opportunity || (leadId && opportunity.leadId !== leadId) || (assessmentId && opportunity.assessmentId !== assessmentId)) throw new Error("booking_opportunity_mismatch");
        }

        const duplicateWindow = await tx.booking.findFirst({
          where: { email: parsed.data.email.toLowerCase(), startsAt, status: { in: ["requested", "confirmed"] } },
          select: { id: true, opportunityId: true },
        });
        if (duplicateWindow) return { id: duplicateWindow.id, duplicate: true, opportunityId: duplicateWindow.opportunityId };

        const created = await tx.booking.create({ data: {
          organizationName: parsed.data.organizationName, contactName: parsed.data.contactName, email: parsed.data.email,
          startsAt, timezone: parsed.data.timezone, notes: parsed.data.notes, source: "website", idempotencyKey,
          leadId, assessmentId, opportunityId,
        }, select: { id: true } });
        ...
        await tx.auditEvent.create({ data: { action: "booking.created", resourceType: "booking", resourceId: created.id, requestId, metadata: { source: "website", leadId, assessmentId, opportunityId } } });
        return { id: created.id, duplicate: false, opportunityId };
      });
    } catch (error) {
      if (idempotencyKey && error instanceof Prisma.PrismaClientKnownRequestError && error.code === "P2002") {
        const existing = await db.booking.findUnique({ where: { idempotencyKey }, select: { id: true, opportunityId: true } });
        if (!existing) throw error;
        result = { id: existing.id, duplicate: true, opportunityId: existing.opportunityId };
      } else throw error;
    }

    if (!result.duplicate) {
      try { await recordGrowthEvent({ eventName: "booking_completed", source: "website", path: new URL(request.url).pathname, entityId: result.id, privacyClass: "PERSONAL" }); } catch (error) { console.error("growth_event_record_failed", { requestId, eventName: "booking_completed", error }); }
    }
