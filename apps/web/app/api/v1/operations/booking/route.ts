...
function isBookableSlot(date: Date) {
  const day = date.getUTCDay();
  const hour = date.getUTCHours();
  const minute = date.getUTCMinutes();
  return day !== 0 && day !== 6 && hour >= 9 && hour < 17 && minute % 30 === 0;
}
  ...
  try {
    ...
    try {
      result = await db.$transaction(async (tx) => {
        ...
        if (opportunityId) {
          const opportunity = await tx.opportunity.findUnique({ where: { id: opportunityId }, select: { id: true, leadId: true, assessmentId: true } });
