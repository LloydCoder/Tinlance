...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const findings = await db.$queryRaw<Array<Record<string, unknown>>>(Prisma.sql`SELECT id,title,description,category,severity,status,recommendation,visibility,authorship,"idempotencyKey",createdAt,updatedAt FROM "WorkspaceFinding" WHERE "organizationId"=${auth.principal.organizationId} AND "idempotencyKey" LIKE ${`m8:${id}:%`} ORDER BY "createdAt"`);
  const traces = await db.$queryRaw<Array<Record<string, unknown>>>(Prisma.sql`SELECT id,"traceId","parentSpanId","spanType",name,"startTime","endTime",status,metadata,"inputHash","outputHash" FROM "EvaluationTrace" WHERE "runId"=${id} AND "organizationId"=${auth.principal.organizationId} ORDER BY "startTime"`);
  return ok(request, { ...runs[0], results, findings, traces });
}
