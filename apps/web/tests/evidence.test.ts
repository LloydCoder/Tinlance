...
describe("evidence taxonomy", () => {
  it("defines every public evidence status with a meaning", () => {
    expect(EVIDENCE_STATUSES).toEqual(["IMPLEMENTED", "TESTED", "VALIDATED", "EXPERIMENTAL", "PLANNED"]);
  ...
  it("fails unknown status values safely to PLANNED", () => expect(evidenceStatus("not-a-status")).toBe("PLANNED"));
...
describe("public architecture registry", () => {
  it("contains every required module", () => {
    expect(architectureModules.map((module) => module.id)).toEqual(["M1", "M3", "M4", "M5", "M6", "M7", "M8", "M9", "M10", "M11", "M12", "M13", "M14"]);
...
describe("case-study taxonomy", () => {
  ...
  it("uses only safe public source URLs", () => {
    for (const study of caseStudies) {
      expect(study.public).toBe(true);
      expect(study.sourceUrl).toMatch(/^https:\/\//);
      expect(study.sourceUrl).not.toMatch(/(localhost|127\.0\.0\.1|private|secret|token)/i);
    }
  });
});

describe("public repository links", () => {
