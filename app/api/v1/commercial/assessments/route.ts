...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!idempotencyKey || idempotencyKey.length < MIN_IDEMPOTENCY_KEY_LENGTH || idempotencyKey.length > MAX_IDEMPOTENCY_KEY_LENGTH || !/^[A-Za-z0-9._:-]+$/.test(idempotencyKey)) {
      return response({ error: "invalid_idempotency_key", requestId }, 400, requestId);
    }
    ...
    try {
      ...
      return response({ error: "invalid_json", requestId }, 400, requestId);
    }

    if (typeof raw === "object" && raw !== null && "websiteTrap" in raw && typeof (raw as Record<string, unknown>).websiteTrap === "string" && (raw as Record<string, unknown>).websiteTrap) {
      return response({ status: "accepted", requestId }, 202, requestId);
    }
